...
- (void)downLoadProgress:(CGFloat)progress {
    ...
    self.progressLabel.text = [NSString stringWithFormat:@"下载进度为:%@%%",@(progress*100)];
}
#pragma mark - 通知
/**
 *  添加播放器通知
...
- (void)playAction:(id)sender {
    ...
    if (!self.player) {
        // 拿到本地的m3u8文件进行播放
        self.playerItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:@"http://localhost:9999/m3u8Movie/movie.m3u8"]];
